...
local v58 = game:GetService("RunService").RenderStepped:Connect(function()
    ...
    while true do if (v223 == (0 -0)) then
        v56+=(1 -0) if ((tick() -v55)>=(1183 -(229 + 953))) then
            local v504 = 1774 -(1111 + 663)
            while true do if (v504==(1580 -(874 + 705))) then
                v56=0 + 0 
                break
            end
        end
...
if (v223==(680 -(642 + 37))) then
    v0:SetWatermark(("Hacker Hub | Murder VS Sherif Duels | %s fps | %s ms"):format(math.floor(v57),math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())))
    break
        end
    end
end)

v59:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind",{Default="End",NoUI=true,Text="Menu keybind"})
v0.KeybindFrame.Visible = false
_G.ShowKb = false

v59:AddToggle("MyToggle",{Text="Show Keybind",Default=false,Tooltip="If you enable Show Keybind it then if u disable it.",Callback=function(v225)_G.ShowKb=v225
    if (_G.ShowKb == false) then
        v0.KeybindFrame.Visible = false
    elseif (_G.ShowKb == true) then
        v0.KeybindFrame.Visible = true
    end
end})

v0.ToggleKeybind = Options.MenuKeybind
v1:SetLibrary(v0)
v2:SetLibrary(v0)
v2:IgnoreThemeSettings()
v2:SetIgnoreIndexes({"MenuKeybind"})
v1:SetFolder("MyScriptHub")
v2:SetFolder("MyScriptHub/specific-game")
v2:BuildConfigSection(v45["UI Settings"])
v2:LoadAutoloadConfig()
v0:Notify("[Notifier] Press End To Close And Open");
