if IY_LOADED and not _G.IY_DEBUG == true then
	-- error("Hacker Admin is already running!", 0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Hacker Admin is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Hacker Admin v" .. currentVersion
...
Logo.Image = "rbxassetid://16103628297"
...
PluginsHint.Text = "Download plugins from the Hacker#3331 Discord (discord.gg/V67aAkXMWf)"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with HA,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		inviteButton.Text = "Copy Discord Invite Link (discord.gg/V67aAkXMWf)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Hacker Admin Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if State == Enum.TeleportState.Started then
		if KeepInfYield and queueteleport then
			queueteleport("loadstring(game:HttpGet(“https://raw.githubusercontent.com/Hacker3331/HackerHub-Games/main/HackerAdminGames.lua”))()")
		end
	end
end)
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if toClipboard then
		toClipboard('https://discord.com/invite/V67aAkXMWf')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/V67aAkXMWf')
	else
		notify('Discord Invite', 'discord.gg/V67aAkXMWf')
	end
	if httprequest then
		httprequest({
...
addcmd('explorerpc', {'dexpc'}, function(args, speaker)
    notify('Loading', 'Hold on a sec')
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Hacker Admin may have issues or features that are not functioning correctly.") end
end)
